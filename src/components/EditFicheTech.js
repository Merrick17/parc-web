...
import { updateFicheTechApi } from "../store/actions/ficheTech.actions";
...
const EditFicheTech = () => {
  ...
  const listVehicule = useSelector((state) => state.vehicule).list;

  const getSelectedFicheTech = () => {
    if (list.length != 0) {
      let selectedFicheTech = list.find((elm) => elm._id == params.id);
      console.log("Selected FicheTech", selectedFicheTech);
      setValue("date", selectedFicheTech.date);
      setValue("content", selectedFicheTech.content);
      setValue("vehicule", selectedFicheTech.vehicule._id);
    }
  };
  const onSubmit = (data) => {
    dispatch(updateFicheTechApi(params.id, data));
    navigate(-1);
  };
  useEffect(() => {
    console.log("Params", params._id);
    getSelectedFicheTech();
  }, [params]);
  return (
        ...
        <div className="col-auto " style={{ minWidth: "100% " }}>
          <div className="card card-primary">
            <div className="card-header">
              <h3 className="card-title">Edit Fiche Technique </h3>
            </div>
            {/* /.card-header */}
            {/* form start */}
